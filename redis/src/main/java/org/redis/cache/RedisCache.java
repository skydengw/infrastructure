...
public class RedisCache implements Cache {
	...
	public ValueWrapper get(Object key) {
		RedisCacheElement element = get(new RedisCacheKey(key).usePrefix(this.cacheMetadata.getKeyPrefix())
				.withKeySerializer(redisOperations.getKeySerializer()));
		if (element != null) {
			if (this.cacheMetadata.getDefaultExpiration() != 0) {
				redisOperations.expire(key, this.cacheMetadata.getDefaultExpiration(), TimeUnit.SECONDS);
			}
		}
		return element;
	}
	...
	/*
	 * @see org.springframework.cache.Cache#get(java.lang.Object,
	 * java.util.concurrent.Callable) introduced in springframework 4.3.0.RC1
	 */
	public <T> T get(final Object key, final Callable<T> valueLoader) {
		...
		BinaryRedisCacheElement rce = new BinaryRedisCacheElement(
				new RedisCacheElement(new RedisCacheKey(key).usePrefix(cacheMetadata.getKeyPrefix())
						.withKeySerializer(redisOperations.getKeySerializer()), valueLoader),
				cacheValueAccessor);
	 ...
	 * 
	 * @param cacheKey
	 *            the key whose associated value is to be returned via its
	 *            binary representation.
	 * @return the {@link RedisCacheElement} stored at given key or
	 *         {@literal null} if no value found for key.
	 * @since 1.5
	 */
	public RedisCacheElement get(final RedisCacheKey cacheKey) {
		...
		return (bytes == null ? null
				: new RedisCacheElement(cacheKey, cacheValueAccessor.deserializeIfNecessary(bytes)));
	}
	...
	/**
	 * Add the element by adding {@link RedisCacheElement#get()} at
	 * {@link RedisCacheElement#getKeyBytes()}. If the cache previously
	 * contained a mapping for this {@link RedisCacheElement#getKeyBytes()}, the
	 * old value is replaced by {@link RedisCacheElement#get()}.
	 * 
	 * @param element
	...
	public void put(RedisCacheElement element) {
		...
		redisOperations.execute(
				new RedisCachePutCallback(new BinaryRedisCacheElement(element, cacheValueAccessor), cacheMetadata));
	}
	 ...
	 * {@link RedisCacheElement#getKeyBytes()}. If a value is present for
	...
	public ValueWrapper putIfAbsent(RedisCacheElement element) {
		...
		return toWrapper(cacheValueAccessor
				.deserializeIfNecessary((byte[]) redisOperations.execute(new RedisCachePutIfAbsentCallback(
						new BinaryRedisCacheElement(element, cacheValueAccessor), cacheMetadata))));
	}
	...
	/**
	 * @param element
	 *            {@link RedisCacheElement#getKeyBytes()}
	 * @since 1.5
	 */
	public void evict(final RedisCacheElement element) {
		...
		notNull(element, "Element must not be null!");
		redisOperations.execute(
				new RedisCacheEvictCallback(new BinaryRedisCacheElement(element, cacheValueAccessor), cacheMetadata));
	}
	...
	/**
	 * {@inheritDoc} This implementation simply returns the RedisTemplate used
	 * for configuring the cache, giving access to the underlying Redis store.
	 */
